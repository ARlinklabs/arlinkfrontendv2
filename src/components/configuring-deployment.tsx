...
import type { DirectoryStructure, ArnsName, BuildSettings, Steps } from "@/types"
...
const ConfiguringDeploymentProject = ({
  ...
  const [isRootDirectoryDrawerOpen, setIsRootDirectoryDrawerOpen] = useState(false)

  async function handleSelectRootDir(path: string) {
    setRootDirectory(path)
    const configPath = path.replace("./", "")
    const config = await getRepoConfig(extractOwnerName(repoUrl), extractRepoName(repoUrl), configPath)

    console.log({
      config,
    })

    const { buildSettings, configFailed, framework } = handleConfigurationAndBuild({
      error: config.error,
  ...
  const [existingArnsLoading, setExistingArnsLoading] = useState<boolean>(true)
  ...
  const handleConfigurationAndBuild = (config: Config, deno?: boolean) => {
    ...
    if (config.error && config.errorType === "static") {
      configState.configFailed = {
        error: true,
        errorMessage: "You are using static files please check the below commands before deploying.",
      }
      ...
      configState.configFailed = {
        error: true,
        errorMessage: "Failed to fetch the configuration, please review the commands below before deploying.",
      }
      configState.buildSettings = {
        installCommand: config.installCommand,
        outPutDir: config.outputDir,
  ...
  useEffect(() => {
    ...
    const handleFetchRepository = async (defaultProjectName: string, owner: string, repo: string) => {
  ...
  async function handleFetchBranches() {
    ...
    try {
      const response = await axios.get<[{ name: string }]>(`https://api.github.com/repos/${owner}/${repo}/branches`, {
        headers: {
      ...
      setSelectedBranch(branchesNames.find((branch) => branch === "main") || branchesNames[0])
    } catch (error) {
  ...
  async function handleArnsSelection(arnsName: ArnsName) {
    ...
    setArnsName(arnsNames.find((arns) => arns.processId === arnsName.processId))
  ...
  const deployProject = async () => {
    ...
    const FAILURE_DELAY = 3000 // 3 seconds delay before stopping logs on failure
    ...
    const startLogPolling = async () => {
      ...
      await new Promise((resolve) => setTimeout(resolve, INITIAL_LOG_DELAY))
      setIsWaitingForLogs(false)
      ...
      pollingIntervalId = setInterval(async () => {
        if (!isPollingActive || Date.now() - startTime > MAX_POLLING_TIME) {
          stopLogPolling()
          if (!isPollingActive) {
    ...
    try {
      ...
      // Set deployment failed after the delay
      setTimeout(() => {
        setDeploymentFailed(true)
      }, FAILURE_DELAY)
    } finally {
  ...
  return (
    <div className="text-white md:px-8 px-4 mb-20 max-w-3xl mx-auto">
      ...
      <h1 className="md:text-2xl text-xl font-bold mb-6">Set up ur deployment process</h1>

      <div className={`rounded-lg ${deploymentStarted ? "opacity-70" : "opacity-100"} mb-6`}>
        <NewDeploymentCard
        ...
        />
        {frameWork.dir.toLocaleLowerCase().includes("next") && <NextJsProjectWarningCard />}

        <div className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-neutral-400 text-sm font-medium mb-1">
              Projects name
            </label>
            <Input
              disabled={deploymentStarted}
          ...
          <div>
            <label htmlFor="branch" className="block text-neutral-400 text-sm font-medium mb-1">
              Branch
            </label>
            {loadingBranches ? (
          ...
          <div>
            <label htmlFor="directory" className="block text-neutral-400 text-sm font-medium mb-3">
              Root directory
            </label>
            <div className="flex relative ">
              ...
              <Button
                ...
                disabled={fetchingSubDir || deploymentStarted}
              >
                {fetchingSubDir ? <Loader2 className="animate-spin" /> : "Change"}
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className={`mb-6 ${deploymentStarted ? "opacity-80 pointer-events-none" : "opacity-100"}`}>
        <DomainSelection
          activeTab={activeTab}
      ...
      >
        <p className="text-md text-neutral-400 font-medium mb-3">Build And Output Settings</p>
        <div className="space-y-4">
          {configFailed.error && (
