...
export default function DeploymentComponent({
  ...
  //   const {} = deployments.find((project) => project.RepoUrl )

  // important/jugadi functions
  // this function is responsible for updating the undername in the onchain

  const updateOnChain = async ({ undername }: { undername: string }) => {
    const alterUndernameQueryResult = await runLua(
      `local res = db:exec[[
    ...
    const fetchDataQueryResult = await runLua(
      `local res = db:exec[[
            SELECT * FROM Deployments
        ]]`,
      managerProcess
    );
    console.log({ onChainData: fetchDataQueryResult });
    console.log("tablealtered", alterUndernameQueryResult);

    const arnsUndernameQueryResult = await runLua(
      `local res = db:exec[[
        UPDATE Deployments
        SET UnderName = '${undername}
        WHERE Name = '${deployment.Name}'
    ]]`,
      managerProcess
    );
    console.log("arns undername updated", arnsUndernameQueryResult);
    refresh();
  };
  ...
  useEffect(() => {
    ...
    const fetchDeploymentUrl = async () => {
      try {
        ...
        deploymentConfigStore.updateDeployment(githubUserPath, response.data);
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        const trimmedLogs = rawLogsData.split("\n").reduce(
          (acc: { started: boolean; logs: string[] }, line: string) => {
    ...
    return (
      <Layout>
        <div className="text-xl">
          Searching <span className="text-muted-foreground">{name} </span> ...
        </div>
      </Layout>
    );
